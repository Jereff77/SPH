...
async def run_test():
    ...
    try:
        ...
        # -> Input username and password, then click login button to log into the application.
        ...
        frame = context.pages[-1]
        # Click the logout button (icon in top right corner) to log out the user
        elem = frame.locator('xpath=html/body/div/div/header/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Verify that protected resources are inaccessible without login by attempting to access a protected page or resource.
