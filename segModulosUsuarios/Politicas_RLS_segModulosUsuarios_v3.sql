-- [Fecha y Hora]: 07/11/2025 07:44:00
-- [Descripción]: Script para implementar políticas RLS corregidas en segModulosUsuarios
--                que evitan la recursión infinita detectada

-- Paso 1: Eliminar políticas actuales
DROP POLICY IF EXISTS segmodulosusuarios_select_propios ON public."segModulosUsuarios";
DROP POLICY IF EXISTS segmodulosusuarios_select_permisos_220 ON public."segModulosUsuarios";
DROP POLICY IF EXISTS segmodulosusuarios_insert_permisos_220 ON public."segModulosUsuarios";
DROP POLICY IF EXISTS segmodulosusuarios_update_permisos_220 ON public."segModulosUsuarios";
...
-- Comentario final
-- Estas políticas evitan la recursión infinita al:
-- 1. No usar EXISTS en la condición de la misma tabla que se está modificando
-- 2. Separar la validación de permisos de la condición de la política
-- 3. Usar condiciones explícitas en lugar de subconsultas recursivas