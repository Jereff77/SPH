DROP FUNCTION IF EXISTS public.plan_dispersiones_dinamico(TEXT, TEXT, TEXT);

CREATE OR REPLACE FUNCTION public.plan_dispersiones_dinamico(
    p_id_fideicomiso TEXT,
    p_no_adhesion TEXT,
    p_no_dispersion TEXT DEFAULT NULL
)
RETURNS TABLE (
...
DECLARE
    ...
    -- Días exactos de rendimiento para cada pago
    v_dias_rendimiento INTEGER;
...
BEGIN
    ...
    LOOP
        ...
        LOOP
            ...
            -- Verificar si se debe filtrar por número de dispersión
            -- Solo filtrar si el período NO coincide con la dispersión solicitada
            IF p_no_dispersion IS NOT NULL AND v_numDispersion != p_no_dispersion THEN
                CONTINUE;
            END IF;
            ...
            IF (v_tiene_promocion AND v_periodo.periodo_inicio > v_fin_promocion) OR NOT v_tiene_promocion THEN
                ...
                -- Calcular días exactos de rendimiento
                IF v_pago.fecha <= v_periodo.periodo_inicio THEN
                    v_dias_normales := v_periodo.dias;
                ELSE
                    v_dias_normales := v_periodo.periodo_fin - v_pago.fecha + 1;
                END IF;
                ...
                RETURN QUERY SELECT
                    ...
                    v_periodo.periodo_inicio, v_periodo.periodo_fin, v_dias_normales,
                    v_numDispersion,
            ...
            ELSIF v_periodo.periodo_fin <= v_fin_promocion THEN
                ...
                ELSE
                    v_dias_promocion := v_periodo.periodo_fin - v_pago.fecha + 1;
                END IF;
                ...
                RETURN QUERY SELECT
                    ...
                    v_tipo_periodo, NULL::TEXT,
                    v_pago."idPago", v_pago.monto, v_pago.fecha,
                    v_periodo.periodo_inicio, v_periodo.periodo_fin, v_dias_promocion,
                    v_numDispersion,
                    v_tasa_rendimiento, v_rendimiento_bruto, v_retencion_isr, v_rendimiento_neto, v_rendimiento_sph, v_dispersion_neta,
                    CURRENT_TIMESTAMP::TIMESTAMP;
            ...
            ELSE
                ...
                -- Calcular días exactos de rendimiento para el sub-período de promoción
                IF v_pago.fecha <= v_fecha_inicio_promo THEN
                    v_dias_promocion := v_fecha_fin_promo - v_pago.fecha + 1;
                ELSE
                    v_dias_promocion := v_fecha_fin_promo - v_fecha_inicio_promo + 1;
                END IF;

                -- Calcular días exactos de rendimiento para el sub-período normal
                IF v_pago.fecha <= v_fecha_inicio_normal THEN
                    v_dias_normales := v_fecha_fin_normal - v_pago.fecha + 1;
                ELSE
                    v_dias_normales := v_fecha_fin_normal - v_fecha_inicio_normal + 1;
                END IF;
