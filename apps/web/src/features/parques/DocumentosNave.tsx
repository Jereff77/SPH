 ...
 * KVA, actas… Es el contenido de la pestaña «Documentos» de `NaveKvaModal`.
 *
...
export function DocumentosNave({ idNave, onCambio, onClose }: Props) {
  ...
  function refrescar() {
    onCambio();
    return qc.invalidateQueries({ queryKey: ['kvas', 'docs', idNave] });
  }
  ...
  const subir = useMutation({
    ...
    onError: (e) =>
      setError(
        e instanceof ApiRequestError ? e.message : 'No se pudo subir el documento.',
      ),
  });
