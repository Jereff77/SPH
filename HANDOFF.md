...
> **antes** de `@Get(':uid')` o Nest lo captura como uid). **No amplía la superficie de datos**: con la 220
> ya se veían los permisos de cualquiera uno por uno; esto solo los agrega. Excel de **4 hojas** (Matriz ·
> Catálogo · Detalle · Resumen) generado **en el navegador con ExcelJS por carga diferida**
> (`features/permisos/permisos-export.ts`), mismo patrón que Contabilidad/Kardex. Incluye **inactivos**
> (auditar accesos retirados) y marca con **★** a los `isSupport` (bypass del RBAC: pintar sus toggles haría
> mentir al reporte). **Nuevo: descripción de para qué sirve cada permiso** — las 80 en
> `features/permisos/permisos-descripciones.ts`, redactadas contra los endpoints que exigen cada clave
> (columna en Catálogo/Detalle + nota al pasar el mouse sobre la clave en la Matriz). 📌 **Al dar de alta una
> clave nueva en `segModulos`, agregarla ahí** (si falta, sale sin descripción; no truena). Backlog acordado:
> mover esas descripciones a `segModulos.descripcion` (cambio de esquema, **sin autorizar**) y que el área
> valide la redacción. **Sin cambios de BD/esquema** (solo INSERT changelog id **88**) +
> `APP_VERSION_RAW`→2.58.0. Escalabilidad (skill `revision-escalabilidad`): **APTO** — se corrigió el
> `.range()` faltante en 2 de las 3 lecturas (PostgREST corta en 1000 filas **en silencio**: el reporte
> saldría incompleto sin error); `segModulosUsuarios` sin índice en `uid`/`acceso` queda como BAJA
> (seq scan de 3.4k filas en 0.66 ms). KB: `modulos/configuraciones.md` §5.1/§10/§11 + INDICE.
> Autor: Toribio/Opus 5. ⚠️ Nota de concurrencia: **v2.57.0** (Invitaciones · botón «Copiar link», commit
> `b37d0f4` de **otra sesión**) subió el bundle pero **nunca registró su changelog en BD** — el id 87 salta
> de 2.56.0 a 2.58.0; pendiente de decidir con Jereff si se registra.
> Previa 2026-07-05 (**v2.56.0** — **Trazabilidad de la nave · Historial + desvinculación sin borrado físico**: nueva pestaña **«Historial»** en el editor de nave (Parques) con la **línea de tiempo de la nave** (venta y renta), **reconstruida desde `auditoria`** (endpoint `GET /parques/naves/:idNave/historial`, clave 700; actor del JWT, motivos; **sin bitácora paralela** — no se duplica la auditoría). **⛔ Regla nueva:** desvincular una nave **ya NO borra físico** — es **BAJA LÓGICA** (`status=false`, conserva el histórico), tanto en Ventas (`desvincularNave`) como en Arrendatarios (`liberarNave`), pidiendo un **motivo** (`propiedades.motivoBaja` / `arrenPropiedades.motivoBaja`, auditado; el actor/fecha viven en `auditoria`). Guarda de **negocio**: no desvincular si hay plan **activo** (venta `pdpActivo`; RG/RA contra la **tabla real** `rentaActiva=true`, porque las banderas se desincronizan). **Prerequisito (aplicado):** `v_naves` y `v_disponibilidad` filtran `prop.status=true` — ya no muestran propiedades dadas de baja como "dueño/ocupante fantasma". **Fix de selectores:** Arrendatarios/Planes (20), Fideicomiso/Aportaciones (510) y Soporte a inquilinos ahora **excluyen la papelera** (`pruebas=false`), como ya hacían Ventas y CxP. Migraciones `2026-07-05-vistas-prop-status.sql`, `2026-07-05-prop-motivo-baja.sql`, `2026-07-04-arren-motivo-baja.sql`; `@erp/types` regenerado. **Validado adversarialmente (Opus): `idPdp=null` en la baja lógica NO falsea pagos/dispersiones** (todo cálculo llega al plan por `idPropiedad`/`pdpDetalle.idPdp`, no por `propiedades.idPdp`; las 2 vistas que lo usan filtran `status=true`). Requiere **redeploy api+web**. Autor: Claude (Opus 4.8). Previa **v2.55.0** (2026-07-03) — **Agente de IA de Soporte · Rediseño razonador**: el agente ahora **razona y resuelve/guía** (consulta la BD vía text-to-SQL acotado por permisos, analiza y responde) en vez de escalar casi todo a ticket; se **retiraron los enlatados** de diagnóstico (`DiagnosticoService` eliminado). **Modelo → `anthropic/claude-sonnet-5`**; **edge `soporte-chat` sin `temperature`** (causaba 502/400 con los modelos nuevos). **Base de Conocimiento reformateada al formato canónico** del router: frontmatter con listas en UNA línea, `claves_permiso` numéricas, y sección **«Para el agente de soporte»** en los **19** módulos. Config en BD: `SOPORTE_IA_MODELO` + `SOPORTE_IA_PROMPT`. Requiere **redeploy api+web + deploy edge `soporte-chat`**. Se creó la skill global `kb-agente-soporte` para el «documenta todo». Previa **v2.38.0** — **Arrendatarios · Gestión de Pagos — saldo a favor, conceptos y registro**: la aplicación de pagos dejó de ser "exacta": **saldo a favor por depósito** (aplica partidas **completas de cualquier mes** del arrendatario; el remanente queda **disponible en el mismo depósito**; reutiliza la RPC `aplicar_pago_arrendatario` **sin tocar BD**, ajustando `movbancarios.aplicado` por saldo); **tolerancia de centavos** configurable (`SPHConfiguraciones.ARRE_TOLERANCIA_PAGO`, default 0.05); **modal** con todas las partidas pendientes (cualquier mes) por **mes→nave** (comprimido), **selector de año** y depósito **derivado de la lista** (fix del saldo); **agregar concepto libre** (caso 3: penalización/interés — `pm2=monto/(1+IVA)`,`constM2=1` porque **`cantidad` es columna GENERADA** = pm2×constM2 → cantidad+iva = monto exacto); **quitar sugerencia** ⭐; **Registro de movimientos** (📋) con filtro **año/mes** (periodo aplicado), columnas **Mes/Año aplicado**, **filtros por columna** (7c) y **export a Excel**. ⚠️ Único cambio en BD: fila de config `ARRE_TOLERANCIA_PAGO` (aditiva). Previa **v2.37.0** — **Ventas — saldos vencidos por cuenta corriente FIFO + filtros multi-selección**: (1) el saldo vencido (Dashboard gráfico 620 + reporte de Vencidos) se calcula con el nuevo **`SaldosVencidosService`** (FIFO por plan: el sobrepago de una parcialidad cubre las demás del **mismo** `idPdp`); antes cada parcialidad se evaluaba aislada e inflaba la cartera (prod **$67.4M→$23.2M**); Dashboard y reporte ahora **cuadran**; las 3 RPCs `v_pdpdetalle_get_saldos_vencidos_*` quedaron **obsoletas** (la vista `v_pdpdetalle` NO se tocó: la usan Estado de Cuenta + filtros). (2) **Filtros multi-selección (regla 7c)** en los reportes de **Ventas, Arrendatarios, Fideicomiso y CxP** (`MultiSearchSelect`, query repetido `?x=a&x=b`, backend filtra en memoria o `.in()`, **sin tocar BD**; los selectores de cascada que cargan datos quedan únicos). (3) **Regla §1 reescrita**: v1 apagado → «**reutilizar antes que duplicar, con análisis de impacto**». ⚠️ Falta `git push` a `erp_v2` (en hold por «no push hasta revisar») + redeploy api+web + changelog v2.37.0 post-deploy. Previa **v2.35.0** — **Versionado**: la versión visible ahora sale del **bundle** (`APP_VERSION`), no de la BD (antes el changelog la cambiaba sin recargar, anulando su propósito) + **banner "nueva versión disponible"** (compara el bundle vs `/version.json` que emite el plugin de Vite; instrucciones Ctrl+Shift+F5 / ⌘+Shift+F5). **Regla nueva (§1.10): graphify solo se refresca con orden EXPLÍCITA del usuario.** Previa **v2.34.2** — **CxP · Aprobar**: corrige el **rechazo** de solicitudes (daba "Error interno del servidor"): `rechazar()` ahora envía `autorizo`=aprobador del JWT (el trigger `cxp_trigger_validar_fecha` valida la clave 430 contra `COALESCE(autorizo,uidr)`; sin él validaba al solicitante → 500) + nuevo helper `common/filters/cxp-error-mapper.ts` que traduce los 8 errores `CXP_*` de los triggers a 403/400 con mensaje claro. **Clientes**: trigger BD `v2_trg_inversionista_razonsocial` autocompleta la razón social de **personas físicas** con su nombre y normaliza `personalidad` a `'Fisica'/'Moral'` (sin acento, compat v1) → los clientes físicos ya aparecen en Devoluciones. Requiere redeploy **api+web**. Previa **v2.34.1** — Fideicomiso · **Contabilidad**: corrige la edición de celdas — al editar ya no se crea un movimiento duplicado (backend `buscarRegistro` localiza con la MISMA normalización del pivote `NULLIF(TRIM(x),'-')`: cubre `subconcepto/descripcion` en NULL o con espacios) y los conceptos **partidos por IVA** vuelven a ser editables (`rowKey` del front incluye `aplicaIVA`); **saneo de datos autorizado**: 6 registros canonizados + 2 conceptos con IVA unificado. Cambia `apps/api`+`apps/web` → requiere redeploy **api+web**. Previa **v2.34.0** — CxP · **Solicitudes de Pago PPD**: el listado resalta facturas con REP **vencido** (rojo, >15 d) / **pendiente** (ámbar) vía `repVencido`/`repPendiente`; columna **Proveedor** acotada con *wrap* y **Folio** completo. **Migración (BD, autorizada):** 25 facturas viejas `diferido=true` sin maestro PPD regularizadas (`cxp_ppd` + 27 parciales ligadas, prefijo `mig…`, reversible) → ya se pueden **dispensar** desde `/cxp/ppd` (permiso 403, hoy solo Carlos Carreón). **Regla de negocio:** una PPD puede pagarse en una sola exhibición y **sí requiere REP**. Previa v2.33.1 — CxP: **Aprobar Solicitudes** muestra la **justificación del solicitante**). Notas: v2.30.0–v2.33.0 (Claves SAT, Vencimientos, Ventas/Fideicomiso, filtros multi-selección, IVA en Gestión de Pagos) las publicaron otras sesiones en paralelo; ver `.sessions/bitacora.md` y `v2_changelog`. Autor: Claude (Opus 4.8).
>
...
- **Hecho hasta ahora (verificado; desplegado en EasyPanel desde la rama `erp_v2`):**
  ...
  - **Arrendatarios** (grupo propio) — **módulo completo**: **Dashboard de cobranza** (clave 10, `/arrendatarios`)
    ...
    **Reportes** (`/arrendatarios/reportes`, clave 20): **Estado de Cuenta** **por nave y plan** (se elige
    Parque→Nave→Plan vigente/terminado; muestra la cabecera del `arrePdp` y la **corrida completa**
    `arrePdpDetalle` desglosada por partida: Renta/Admin/Mtto/Vig + Otros Servicios + Nota + Total + Pagado/
...
- **Despliegue:** EasyPanel, 2 apps (api Dockerfile `apps/api/Dockerfile` :3001, web `apps/web/Dockerfile`
  :80). Guía: `DEPLOY-EASYPANEL.md`. Flujo: **push a `erp_v2` → redeploy AUTOMÁTICO** (auto-deploy webhook; NO
  se pulsa "Implementar" a mano). ⇒ no pushees cambios triviales/solo-docs si no quieres un rebuild.
   ...
   **changelog versionado** visible en **Configuraciones → Novedades** (sin permiso, para **todos** los
   ...
   - **🗣️ Comando «documenta todo».** Cuando el usuario diga **«documenta todo»**, el agente ejecuta el
     ...
     **commit+push** en `erp_v2`). **⛔ graphify NO forma parte del cierre por defecto:** solo se
     ...
     - **✅ Autorización del changelog (permanente, indicada por el usuario 2026-06-22):** el comando
       «documenta todo» **pre-autoriza por sí mismo** el registro de la versión en `public.v2_changelog`
       (INSERT/`v2_changelog_registrar`). Es la **única** escritura a BD que el cierre autoriza sin pedir
       confirmación aparte (excepción explícita a la regla 1); el resto de mutaciones de BD siguen su norma.
     - **🚀 Redeploy automático (permanente, indicado por el usuario 2026-06-22):** el `git push` a `erp_v2`
       **dispara el redeploy de api+web automáticamente** (auto-deploy webhook de EasyPanel); NO hay que pulsar
       "Implementar". **Corolario:** evita pushear cambios triviales/solo-docs si no quieres un rebuild.
...
- **⛔ Commits/Git (REGLA INVIOLABLE):**
  - **Lo que se sube a `erp_v2` ES el contenido de `version2/`.** La carpeta `version2/` es el **working copy**
    ...
    (de ahí despliega EasyPanel: push a `erp_v2` → **redeploy automático**, sin pulsar "Implementar"). Existe en este entorno para dar acceso al
