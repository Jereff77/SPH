--[Fecha y Hora]: 13/11/2025 18:52:30
--[Descripción]: Obtiene datos únicos para filtros dependientes en CXP (versión corregida)
--
--[Parámetros]:
...
--               donde las opciones dependan de selecciones previas
--
--[Ejemplo]: 
--   -- Obtener categorías y secciones de un proveedor específico
--   SELECT * FROM cxp_get_filtros_dependientes_v2('PROVEEDOR XYZ', NULL, NULL);
--   
--   -- Obtener secciones de una categoría específica
--   SELECT * FROM cxp_get_filtros_dependientes_v2(NULL, 'Mantenimiento', NULL);
--
--[Relaciones]: 
--   - Tabla principal: cxp
...
--   - Usa TABLAS CORRECTAS (PresCategorias en lugar de catCategorias)

CREATE OR REPLACE FUNCTION public.cxp_get_filtros_dependientes_v2(
...
BEGIN
    ...
    -- [Uso]: SELECT * FROM cxp_get_filtros_dependientes_v2('PROVEEDOR XYZ', null, null);
